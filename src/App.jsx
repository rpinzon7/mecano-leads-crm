...
const reminders=useMemo(()=>{const activeLeads=leads.filter((l)=>!["Negocio cerrado","Negocio perdido"].includes(l.estado));return {vencidos:activeLeads.filter((l)=>getReminderBucket(l.fechaSeguimiento)==="Vencidos"),hoy:activeLeads.filter((l)=>getReminderBucket(l.fechaSeguimiento)==="Hoy"),sieteDias:activeLeads.filter((l)=>getReminderBucket(l.fechaSeguimiento)==="7 dias"),sinFecha:activeLeads.filter((l)=>getReminderBucket(l.fechaSeguimiento)==="Sin fecha")}},[leads]);
...
const callQueue=useMemo(()=>[...leads].filter((l)=>l.clientePorLlamarManual||["Negociación","Presupuesto enviado","Cotización proveedor"].includes(l.estado)||l.propuestaEstado==="En revisión cliente"||l.propuestaEstado==="Ajustes solicitados").map((l)=>({...l,priority:calculatePriority(l)})).sort((a,b)=>b.priority-a.priority).slice(0,10),[leads]);
